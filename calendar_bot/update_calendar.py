...
from .utils import (
    make_event_from_github_issue_str,
...
def process_issue(
    issue_number: str, body: str, data_dir: Path = Path("data/")
) -> tuple[ics.Event, bool]:
    """Process a GitHub issue by creating or updating a calendar event.
    
    Returns:
        tuple: (event, was_updated) where was_updated is False if no changes were made
    """
    event_uid = f"ghissue_{issue_number}"
    new_event = make_event_from_github_issue_str(event_uid, body)
...
def delete_issue(issue_number: str, data_dir: Path = Path("data/")) -> bool:
    """Delete a calendar event associated with a GitHub issue.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process GitHub issues to manage calendar events"
    )
    parser.add_argument("--issue_number", required=True, help="GitHub issue number")
    parser.add_argument("--body", help="Issue body content (required unless --delete)")
    parser.add_argument(
        "--delete",
        action="store_true",
    ...
    if args.delete:
        delete_issue(args.issue_number, args.data_dir)
    else:
        if args.body is None:
            parser.error("--body is required when not using --delete")
        process_issue(args.issue_number, args.body, args.data_dir)
